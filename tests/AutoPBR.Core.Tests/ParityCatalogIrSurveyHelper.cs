...

{
    ...
    internal readonly record struct SurveyResult(
        ...
        IReadOnlyList<string> CleanRoomPaths,
...

    {
...

        {
            ...
            else if (row.DriverKind == PreviewMeshDriverKind.CleanRoom)

            {

                cleanRoom++;

                cleanRoomPaths.Add(row.TexturePath);
        ...
        var summary = new SurveyResult(paths.Count, ir, cleanRoom, failed.Count, cleanRoomPaths, reasons);
