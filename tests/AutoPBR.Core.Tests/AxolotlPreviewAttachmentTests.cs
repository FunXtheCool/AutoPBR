...
public sealed class AxolotlPreviewAttachmentTests
{
    ...
    [Fact]
    public void Runtime_mesh_gill_attachment_matches_hand_builder_in_preview_space()
    {
        ...
        var hand = CleanRoomEntityModelRuntime.BuildAxolotlHandMeshForTests(TexturePath, Profile26);
        var handPartOrder = new[]
        {
            "body", "head", "top_gills", "left_gills", "right_gills",
            "right_hind_leg", "left_hind_leg", "right_front_leg", "left_front_leg", "tail",
        };
        ...
        foreach (var gillId in new[] { "top_gills", "left_gills", "right_gills" })
        {
            Assert.True(handIdxByPart.TryGetValue(gillId, out var handIdx), gillId);
            var handWorld = hand.Elements[handIdx].LocalToParent;
            Matrix4x4? irWorld = null;
            var irCount = 0;
            for (var i = 0; i < mesh!.Elements.Count; i++)
            {
                ...
                {
                    irCount++;
                    irWorld ??= mesh.Elements[i].LocalToParent;
                }
            }

            Assert.Equal(1, irCount);
            Assert.NotNull(irWorld);
            foreach (var probe in new[] { Vector3.Zero, new Vector3(0f, -2f, 0f), Vector3.UnitX })
            {
                var handPreview = Vector3.Transform(probe, handWorld);
                var irPreview = Vector3.Transform(probe, irWorld.Value);
                _output.WriteLine($"{gillId} probe={probe}: hand={handPreview} ir={irPreview}");
                Assert.True(Vector3.Distance(handPreview, irPreview) <= 0.55f,
                    $"{gillId} probe={probe}: hand={handPreview} ir={irPreview}");
            }
        }
        ...
        for (var v = 0; v < gpuVerts.Length / stride; v++)
        {
            ...
            var p = EntityEmulatedGpuSkinningMath.PreviewCuboidNormalizeTexelPosition(
                new Vector3(gpuVerts[v * stride], gpuVerts[v * stride + 1], gpuVerts[v * stride + 2]));
            var id = partIds[bi];
            if (id.Contains("head", StringComparison.Ordinal) && !id.Contains("gill", StringComparison.Ordinal))
            {
                headY = MathF.Max(headY ?? float.NegativeInfinity, p.Y);
            }
            else if (id.Contains("top_gills", StringComparison.Ordinal))
            {
                topGillY = MathF.Min(topGillY ?? float.PositiveInfinity, p.Y);
            }
        }

        _output.WriteLine($"gpu headMaxY={headY:F4} topGillMinY={topGillY:F4} gap={topGillY - headY:F4}");
        Assert.True(topGillY is not null && headY is not null);
        Assert.True(topGillY.Value <= headY.Value + 0.5f, $"top gills hover above head (gap={topGillY - headY:F3})");
    }
