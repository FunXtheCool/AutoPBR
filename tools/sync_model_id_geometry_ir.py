...
EntityModelRuntime parity cuboids (rig-space cuboids; notes cite builder methods).
...
def blaze_roots() -> list[dict]:
    """EntityModelRuntime.BuildBlaze (rodSpin=0)."""
    children = [
        {
            "id": "head",
            "pose": T(8, 14, 8),
...
def main() -> None:
    ...
    for short in lines:
        ...
        notes_header = [
            "Synced from EntityModelRuntime for Model-ID.txt batch (tools/sync_model_id_geometry_ir.py)."
        ]
        ...
        if short in (
            ...
            notes = notes_header + [
                f"{fqn}: no mesh-definition factory on this class (e.g. createBodyLayer) and no EntityModelRuntime "
        ...
        if short == "BlazeModel":
            roots = blaze_roots()
            notes = notes_header + [
                "Mirrors EntityModelRuntime.BuildBlaze (rodSpin=0); legacy flat FQN shard aligned to packaged blaze mesh."
            ]
        ...
        if short == "VillagerLikeModel":
            ...
            notes = notes_header + [
                "Roots mirrored from VillagerModel.json (EntityModelRuntime.BuildVillager baseline family)."
            ]
            save_shard(path, base, roots, "ok", notes)
            updated += 1
            continue
        ...
        if short in ("AbstractFelineModel", "AdultOcelotModel", "BabyCatModel", "BabyFelineModel", "BabyOcelotModel"):
            ...
            notes = notes_header + [
                "Feline family mesh aligned with EntityModelRuntime.BuildCat (same FelineModel.createBodyLayer topology)."
            ]
            save_shard(path, base, roots, "ok", notes)
            updated += 1
            continue
        ...
        if short == "EquineSaddleModel":
            ...
            notes = notes_header + [
                "Hand-authored from EntityModelRuntime.BuildEquineSaddle; ApplyEquineLivingEntityRendererPreviewBasis may adjust world basis in preview."
            ]
            save_shard(path, base, roots, "partial", notes)
            updated += 1
            continue
        ...
        if short == "SheepFurModel":
            ...
            notes = notes_header + [
                "Wool-only slice from EntityModelRuntime.BuildSheep (SheepModel fur layer / body shell)."
            ]
            save_shard(path, base, roots, "ok", notes)
            updated += 1
            continue
        ...
        if short in ("AbstractPiglinModel", "AdultZombifiedPiglinModel", "ZombifiedPiglinModel"):
            ...
            notes = notes_header + [
                "Piglin-family IR copied from PiglinModel.json; EntityModelRuntime.BuildZombifiedPiglin delegates to BuildPiglin."
            ]
            save_shard(path, base, roots, "partial", notes)
            updated += 1
            continue
        ...
        if short == "AbstractZombieModel":
            ...
            notes = notes_header + [
                "Abstract zombie mesh baseline copied from ZombieModel.json (EntityModelRuntime.BuildZombieHumanoid family)."
            ]
            save_shard(path, base, roots, "partial", notes)
            updated += 1
            continue
        ...
        if short == "GiantZombieModel":
            ...
            notes = notes_header + [
                "EntityModelRuntime routes giant zombies through BuildHumanoid (canonical 64×64 biped boxes)."
            ]
            save_shard(path, base, roots, "ok", notes)
            updated += 1
            continue
        ...
        if short == "PiglinHeadModel":
            roots = copy_roots_from("net.minecraft.client.model.object.skull.SkullModel.json")
            notes = notes_header + [
                "Piglin head IR temporarily mirrored from SkullModel.json; EntityModelRuntime.BuildPiglinSkull uses piglin-specific UV stack."
            ]
            save_shard(path, base, roots, "partial", notes)
            updated += 1
            continue
        ...
        if short == "CopperGolemStatueModel":
            ...
            notes = notes_header + [
                "Statue layer copied from animal.golem.CopperGolemModel.json (EntityModelRuntime.BuildCopperGolem mesh)."
            ]
            save_shard(path, base, roots, "partial", notes)
            updated += 1
            continue
    ...
    if extra_blaze.is_file():
        ...
        save_shard(
            ...
            "ok",
            [
                "Synced from EntityModelRuntime.BuildBlaze (rodSpin=0) via tools/sync_model_id_geometry_ir.py.",
