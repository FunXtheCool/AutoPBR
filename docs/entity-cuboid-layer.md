# Entity cuboid layer (preview)

Hand-built CleanRoom entity meshes were **removed in 2026-07**; catalogued vanilla entities use bundled geometry IR exclusively. This doc covers the cuboid emit layer and IR parity path that remain in production.

Data-first cuboids sit between bytecode-lifted geometry IR / codegen tables and `RigBuilder.AddBox`. **Lifted IR is authoritative** for static body-layer geometry; uncatalogued or IR-failure paths emit a source-style error mesh (`PreviewMeshDriverKind.ErrorPlaceholder`).

## 1. Data-first cuboid records (`EntityCuboid`)

**Location:** `EntityShared.cs` — `EntityCuboid`, `EmitCuboids`, `QuadrupedLegCuboidTex016`.
...
**IR-only catalog path:**

| Entry | File | Notes |
|-------|------|-------|
| Parity catalog | `EntityGeometryIrParityCatalog.cs` | `TryBuildParityCatalogMeshFromGeometryIr` — sole mesh path for manifest rows |
| Dispatch | `EntityDispatch.cs` | Catalog → geometry IR; failure → `EntityPreviewErrorMesh` |
...
**Entry:** `EntityDispatch.TryDispatchEntityStaticMeshBuild` → `TryBuildParityCatalogMeshFromGeometryIr`.

**Status (26.1.2 catalog):** All **761** manifest diffuse paths report `PreviewMeshDriverKind.RuntimeGeometryIrJson` in `ParityCatalogMeshDriverKindSurveyTests` (strict `ErrorPlaceholder == 0` gate). Hand-built mesh fallbacks were removed in 2026-07.
...
- **`GeometryIrLiftPolicy`**: cuboids must be `liftKind: exact` (empty `faceMask` cuboids skipped at emit). Approximate lifts fail IR emit and surface as error placeholders when the path is catalogued with suppressed fallback.
...
**Object-entity orientation:** After IR emit, `EntityBlockEntities.cs` applies preview Y-up correction (bed facing, boat/sign flip, decorated-pot cap/rim/ground-lift) via `EntityGeometryIrParityCatalog` finish passes.

**Tree repair:** `GeometryIrPartTreeRepair` reparents known flat IR mistakes (Chicken `beak` / `red_thing` under `head`) before emit.

**Diagnostics:** `EntityModelRuntime.ClassifyParityCatalogGeometryIrFailure` mirrors atlas/emit steps for survey taxonomy (`atlas_failed`, `emit_failed:*`, `empty_elements`).
...
- [`Dynamic-Runtime-Geometry-Roadmap.md`](Dynamic-Runtime-Geometry-Roadmap.md) — resolver milestone (CleanRoom phase-out complete)
